...
def print_results(results_dic, results_stats_dic, model, 
    ...
    for stat in results_stats_dic:
        if "pct" in stat:
            if "_dogs" in stat:
                print("The correct dogs percentage {}%".format(results_stats_dic[stat]))
            elif "breed" in stat:
                print("The correct breed percentage {}%".format(results_stats_dic[stat]))
            elif "notdogs" in stat:
                print("The correct not dog percentage:{}%".format(results_stats_dic[stat]))
  
    #Misclassified dogs
    if print_incorrect_dogs and results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'] != results_stats_dic[ 'n_images']:
        for image in results_dic:
            #print(sum(results_dic[image][3:]))
            if sum(results_dic[image][3:])==1:
                print("Misclassified dogs:")
                for index in range(0,2):
                    print(results_dic[image][index])
                    
    #Miscalssified Breed
    
    if print_incorrect_breed:
        if results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed']:
            print("Missclassified Breed's of Dog:")
            for image in results_dic:
                if sum(results_dic[image][3:])==2 and results_dic[image][2]==0:
                    print(results_dic[image][0])
                    print(results_dic[image][1],'\n')
